          ...
          pool.execute(
            ...
            function (err, result) {
              ...
              response.status(201).json({
                ...
                token: jwt.sign({ userId: result.insertId }, `${secret}`, {
                  expiresIn: `${time}`,
                }),
              });
            }
...
exports.login = (req, response, next) => {
  ...
  pool.execute(
    search,
    [maskemail(req.body.email, { allowed: /@\.-/ })],
    function (err, res) {
      let user = res[0];
      console.log(user);
        ...
        bcrypt
          ...
          .then((hash) => {
            ...
            pool.execute(sql, [req.params.id], function (err, result) {
              ...
              response.status(200).json({
                pseudo: user.pseudo,
                message: `Le compte numéro ${req.params.id} a bien été supprimé`,
              });
            });
          })
          .catch((error) => response.status(500).json({ error }));
        ...
        if (!match) {
          ...
          bcrypt
            ...
            .then((hash) => {
              ...
              pool.execute(
                sql,
                [hash, req.body.pseudo, req.params.id],
                function (err, resultat) {
                  if (err) throw err;
                  console.log(resultat);
                  res.status(201).json({
                    message: `Le compte numéro ${req.params.id} a bien été modifié`,
                  });
                }
              );
            })
...
exports.getOne = (req, res, next) => {
  let one = `SELECT id, email, pseudo FROM groupomania.user WHERE user.id=?;`;
...
exports.getAll = (req, res, next) => {
  let all = `SELECT id, email, pseudo FROM user;`;
